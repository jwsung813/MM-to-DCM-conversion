...
% This script is designed to function when there is no time interval between MM binary files. 
% If a time interval exists, this script will pause automatically. 
% To resume, delete MM files after the file with the long interval (>1.6 min), and click any key. The script will then resume operation.
...
%% Find the time interval between MM binary files 
...
%acquisition_timeinterval = diff(acquisition_timepoints);
...
if max_interval > 10^5 % over 1.66 min
...
%% Classification of axial, coronal, and sagittal plane 
...
for plane = 1:3
    ...
    for ff = 1:size(files,1)
        ...
        ImagePositionPatient(1) = -image_spacing(1)*(image_size(1) - 1)/2 - 19.53;
        ImagePositionPatient(2) = -image_spacing(2)*(image_size(2) - 1)/2 - 7.24;
        ImagePositionPatient(3) = -15.9;
        ...
        info.Modality = 'MR';
        info.SliceLocation = ImagePositionPatient(3);
        if plane == 1 
